...
def get_urls(path = 'history.csv', verbose=True):
    ...
    with open('priceRAG/scrape/content/urls_ebay.txt', 'w') as f:
        for item in unique_item_urls_ebay:
            f.write("%s\n" % item)
            
    with open('priceRAG/scrape/content/urls_dba.txt', 'w') as f:
        for item in unique_item_urls_dba:
            f.write("%s\n" % item)

    with open('priceRAG/scrape/content/urls_facebook.txt', 'w') as f:
